...
class AppFinancasSupabaseUser extends BaseAuthUser {
  AppFinancasSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> appFinancasSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AppFinancasSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
